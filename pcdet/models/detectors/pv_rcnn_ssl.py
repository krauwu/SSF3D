...
from pcdet.ops.pointnet2.pointnet2_stack.pointnet2_utils import farthest_point_sample
...
class PVRCNNSSL(Detector3DTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if 'pseudo_label_flag' in batch_dict.keys():
            ...
            batch_dict['teacher_anchor_head_cls'] = None
            batch_dict['teacher_point_head_cls'] = None
            batch_dict['teacher_roi_head_cls'] = None
            with torch.no_grad():
        ...
        # train
            ...
            labeled_idx_mask = [True if int(frame) in self.label_idx else False for frame in batch_frame_id] #[True, False, True, ...](batchsize)
            batch_dict['labeled_frame'] = labeled_idx_mask
            batch_dict['teacher_anchor_head_cls'] = None
            batch_dict['teacher_point_head_cls'] = None
            batch_dict['teacher_roi_head_cls'] = None
            # print('batch_dict',batch_dict['gt_boxes'],labeled_idx_mask,batch_dict['frame_id'])
            ...
            with torch.no_grad():
                ...
                anchor_teacher_cls = batch_dict['teacher_anchor_head_cls'].detach()
                point_teacher_cls = batch_dict['teacher_point_head_cls'].detach()
                # end_proposal_cls = batch_dict['batch_cls_preds'].sigmoid().detach() # (bs, POST_NUM, 1)
                # end_proposal_label = batch_dict['roi_labels'].detach() # (bs, POST_NUM)
                #
                #
                # for label in torch.unique(end_proposal_label):
                #     label = int(label)
                #     scores_gmm = (end_proposal_cls[end_proposal_label == label])
                #     print('scores_add',scores_gmm[scores_gmm > 0.5])
                ...
                # if have dual threshold, replace the points with aug points
                # and for each scene without gt_label, use teacher model to get pseudo label
                if dual_threshold is not None:
                    ...
                    # aug data go through teacher model pipeline
                    for cur_module in self.pv_rcnn_ema.module_list:
                    ...
                    pseudo_label_dict = self.get_pseudo_label.filter_pseudo_label_training(pred_dicts, pred_dicts_aug,
            ...
            # points_dyn = batch_dict_train['points'].cpu()
            # boxes_dyn = batch_dict_train['gt_boxes'].cpu()
            # epoch_now = batch_dict_train['cur_epoch'][0] - 80
            # batch_dict_train['points'] = self.dyn_data_aug(points_dyn, boxes_dyn, epoch_now)
            #
                       # object level augmentation
            # pts_haha = batch_dict_train['points'].cpu()
            # pts_haha_mask = points_in_boxes_cpu(pts_haha[:,1:4], boxes_dyn[0][:,:-1])
            # pts_haha = pts_haha[pts_haha_mask.sum(0) != 0]
            # #
            # pts_dyn_mask = points_in_boxes_cpu(points_dyn[:,1:4], boxes_dyn[0][:,:-1])
            # pts_dyn = points_dyn[pts_dyn_mask.sum(0) != 0]
            ...
            self.pv_rcnn.train()
            # for key,val in batch_dict_train.items():
            #     print(key,"batch_dicts_train_before_student")
            batch_dict_train['labeled_frame'] = labeled_idx_mask
            batch_dict_train['teacher_anchor_head_cls'] = anchor_teacher_cls
            batch_dict_train['teacher_point_head_cls'] = point_teacher_cls
    ...
    @staticmethod
    def dyn_data_aug(points, boxes, cur_epoch):
        batch = torch.unique(points[:, 0]).tolist()
        points_auged_list = []
        rand_list = [0, 1, 2, 3]
        weight_rand = [0.98, 0.01, 0, 0.01]
        ...
        for frame in batch:
            ...
            frame_flag = points[:, 0] == frame
            frame_points = points[frame_flag]
            frame_points = frame_points[:, 1:]
            frame_labels = frame_boxes[:, -1]
            frame_boxes = frame_boxes[:, :-1]
            frame_boxes = frame_boxes[frame_labels != 0]
            points_masks = points_in_boxes_cpu(frame_points[:, 0:3], frame_boxes)
            points_not_in_boxes = frame_points[points_masks.sum(0) == 0]

            for box_num in range(points_masks.shape[0]):
                points_in_boxes_list.append(frame_points[points_masks[box_num] != 0])

            for i in range(len(points_in_boxes_list)):
                rand_choice = np.random.choice(rand_list, p=weight_rand)

                if rand_choice == 0:
                    points_in_boxes_list[i] = points_in_boxes_list[i]
                if rand_choice == 1:
                    if frame_labels[i] == 1:
                        if points_in_boxes_list[i].shape[0] > 200:
                            points_in_boxes_list[i] = layer_wise_points_aug_car(points_in_boxes_list[i], frame_boxes[i])
                    if frame_labels[i] == 2:
                        if points_in_boxes_list[i].shape[0] > 150:
                            points_in_boxes_list[i] = layer_wise_points_aug_ped(points_in_boxes_list[i], frame_boxes[i])
                    if frame_labels[i] == 3:
                        if points_in_boxes_list[i].shape[0] > 150:
                            points_in_boxes_list[i] = layer_wise_points_aug_cyc(points_in_boxes_list[i], frame_boxes[i])
                if rand_choice == 2:
                    if points_in_boxes_list[i].shape[0] > 150:  #
                        points_in_boxes_list[i] = down_sample(points_in_boxes_list[i], frame_boxes[i])

                if rand_choice == 3:
                    points_in_boxes_list[i] = points_offset(points_in_boxes_list[i], frame_boxes[i])

            modi_points = torch.cat(points_in_boxes_list, dim=0)
            modi_points = torch.cat([modi_points, points_not_in_boxes], dim=0)
            modi_points = torch.cat([torch.ones(modi_points.shape[0], 1) * frame, modi_points], dim=1)
            points_auged_list.append(modi_points)

        final_points = torch.cat(points_auged_list, dim=0)

        return final_points.cuda()
...
class GetPseudoLabel:
    ...
    def filter_pseudo_label_training(self, pred_dict, pred_dict_aug, batch_dict, total_rot, total_sca, dual_threshold, label_mask):
        ...
        for bs_idx in range(batch_size):
            ...
            # iou_flag to filter out pseudo-labels that dont have any overlap with gt
            if len(train_box) != 0:
...
class TrainDataProcessor:
    ...
    def get_removal_points_scene_no_label(pred_dicts, batch_dict, label_mask):
        ...
        for index in range(len(batch_dict['gt_boxes'])):
            ...
            # removal the noise points which make teacher label worse
...
def layer_wise_points_aug_car(points, ori_box):
    # start = time.time()
    layers_height = 0.1
    box_layers = int(ori_box[5] / layers_height)

    # create bool list
    bool_list = [True if i % 3 == 1 else False for i in range(box_layers)]

    true_indices = [i for i, val in enumerate(bool_list) if val]
    false_indices = [i for i, val in enumerate(bool_list) if not val]
    change_count = random.randint(int(box_layers/2)-int(box_layers/4), int(box_layers/2) - 1)
    # change_count = 0
    # change_indices = random.sample(false_indices, change_count)
    ...
    for i in range(box_layers):
        tmp_box = torch.tensor(
            [ori_box[0], ori_box[1], ori_box[2] - ori_box[5] / 2 + (i + 1) * layers_height,
             ori_box[3], ori_box[4], layers_height, ori_box[6]])
        tmp_box = torch.unsqueeze(tmp_box, dim=0)
        box_list.append(tmp_box)

    for i, box in enumerate(box_list):
        points_masks = points_in_boxes_cpu(points[:, 0:3], box)
        cur_points = points[points_masks[0] != 0]

        if bool_list[i]:

            cur_points = cur_points.unsqueeze(dim=0).float().cuda()
            cur_pt_idxs = farthest_point_sample(cur_points[:, :, 0:3].contiguous(),
                                                int(cur_points.shape[1] * 0.7)).long()
            cur_points = cur_points[0][cur_pt_idxs[0]].cpu()

            point_list.append(cur_points)
        else:
            continue
    final_pts = torch.cat(point_list, dim=0) if len(point_list) > 0 else torch.tensor([])

    return final_pts.cpu()


def layer_wise_points_aug_ped(points, ori_box):
    layers_height = 0.05
    box_layers = int(ori_box[5] / layers_height)
    # create bool list
    bool_list = [True if i % 2 == 1 else False for i in range(box_layers)]

    true_indices = [i for i, val in enumerate(bool_list) if val]
    false_indices = [i for i, val in enumerate(bool_list) if not val]
    change_count = random.randint(int(box_layers/2)-int(box_layers/4), int(box_layers/2) - 5)
    # change_count = 0
    change_indices = random.sample(false_indices, change_count)
    for i in change_indices:
        bool_list[i] = True
...
def layer_wise_points_aug_cyc(points, ori_box):  # cpu->gpu->cpu
    ...
    for i in range(box_layers):
        tmp_box = torch.tensor(
            [ori_box[0], ori_box[1], ori_box[2] - ori_box[5] / 2 + (i + 1) * layers_height,
             ori_box[3], ori_box[4], layers_height, ori_box[6]])
        tmp_box = torch.unsqueeze(tmp_box, axis=0)
        box_list.append(tmp_box)
    for i, box in enumerate(box_list):
        points_masks = points_in_boxes_cpu(points[:, 0:3], box)
        cur_points = points[points_masks[0] != 0]
...
# cur_points = cur_points.unsqueeze(dim=0).float().cuda()
# cur_pt_idxs = farthest_point_sample(cur_points[:, :, 0:3].contiguous(), 20).long()
# cur_points = cur_points[0][cur_pt_idxs[0]]

def down_sample(points, ori_box):
    if points.shape == torch.Size([0]):
        return points
    # obstacle box init

    tmp_box = torch.tensor(
        [ori_box[0], ori_box[1], ori_box[2],
         ori_box[3], ori_box[4], ori_box[5], ori_box[6]])
    tmp_box = torch.unsqueeze(tmp_box, axis=0)

    down_list = [0.8, 0.9]
    weight = [0.5, 0.5]
    keep_ratio = np.random.choice(down_list, p=weight)

    points_masks = points_in_boxes_cpu(points[:, 0:3], tmp_box)
    cur_points = points[points_masks[0] != 0]

    cur_points = cur_points.unsqueeze(dim=0).float().cuda()
    cur_pt_idxs = farthest_point_sample(cur_points[:, :, 0:3].contiguous(), int(cur_points.shape[1]*0.3)).long()
    # print(cur_points.shape,'dsadas')
    cur_points = cur_points[0][cur_pt_idxs[0]].cpu()

    return cur_points


def points_offset(points, ori_box): # cpu->gpu->cpu

    if points.shape == torch.Size([0,4]):
        return points
    offset_ratio = 0.08
    offset_num = int(points.shape[0] * offset_ratio)
    indices = np.random.choice(points.shape[0], size=offset_num, replace=False)
    select_points = points[indices]
    # not_select_points = [points[i] for i in range(points.shape[0]) if i not in indices]
    # not_select_points = torch.cat([x.unsqueeze(0) for x in not_select_points], dim=0)
    not_select_points = torch.cat([points[i].unsqueeze(0) for i in range(points.shape[0]) if i not in indices], dim=0)
    for i in range(offset_num):
        rand_val_x = random.randint(-5, 5)
        select_points[i][0] += rand_val_x * 0.01
        rand_val_y = random.randint(-5, 5)
        select_points[i][1] += rand_val_y * 0.01
        rand_val_z = random.randint(-5, 5)
        select_points[i][2] += rand_val_z * 0.01
    if len(not_select_points) == 0:
        points = select_points
    else:
        points = torch.cat([select_points, not_select_points], dim=0)
