...
class QualityFocalLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        ...
        pred_sigmoid = torch.sigmoid(input)
        neg_pred = pred_sigmoid.new_zeros(pred_sigmoid.shape)
        quality_bce_loss = F.binary_cross_entropy(pred_sigmoid, neg_pred, reduction='none') * pred_sigmoid.pow(self.gamma) * (1-self.alpha)

        mask = target > 0
        if mask.shape.__len__() == 3:
            mask[mask.any(2)] = True

        pt = target[mask] - pred_sigmoid[mask]

        quality_bce_loss[mask] = F.binary_cross_entropy(pred_sigmoid[mask], target[mask], reduction='none') * pt.pow(self.gamma) * (1-self.alpha)

        loss = quality_bce_loss
