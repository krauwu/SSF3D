...
def merge_result(test_anns, aug_test_anns, cur_epoch, ckpt_save_dir, root_path, label_frame_idx,
    ...
    for category in all_conf_score.keys():
        ...
        if category == 'Cyclist':
            if len(all_conf_score[category]) > 2:
                threshold_pool[category][0] = get_GMM_threshold(all_conf_score[category])[2] if get_GMM_threshold(all_conf_score[category])[2] >= 0.98 else 0.98
                threshold_pool[category][1] = get_GMM_threshold(all_conf_score[category])[1]
            if len(all_roi_score[category]) > 2:
                threshold_pool[category][2] = get_GMM_threshold(all_roi_score[category])[1] if get_GMM_threshold(all_roi_score[category])[1] >= 0.92 else 0.92
                threshold_pool[category][3] = get_GMM_threshold(all_roi_score[category])[1]
            if len(all_roi_score[category]) > 2:
                threshold_pool[category][4] = get_GMM_threshold(all_iou_score[category])[1] if get_GMM_threshold(all_iou_score[category])[1] >= 0.85 else 0.85
