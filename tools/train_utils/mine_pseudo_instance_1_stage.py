...
def get_pseudo_label(model, dataloader, rank, cur_epoch, ckpt_save_dir, dist_test=False, result_dir=None):
    ...
    det_annos = []
    aug_det_annos = []
    for o_batch_dict in tqdm.tqdm(dataloader, desc='get_pseudo_label'):
...
def merge_result(test_anns, aug_test_anns, cur_epoch, ckpt_save_dir, root_path, label_frame_idx,
    ...
    threshold_pool = {'Car': [0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
                      'Pedestrian': [0.96, 0.77, 0.88, 0.45, 0.9, 0.06],
                      'Cyclist': [0.97, 0.55, 0.93, 0.52, 0.95, 0.9]}
    ...
    for category in all_conf_score.keys():
        ...
        if category == 'Pedestrian':
            if len(all_conf_score[category]) > 2:
                threshold_pool[category][0] = get_GMM_threshold(all_conf_score[category])[0] if get_GMM_threshold(all_conf_score[category])[0] > 0.98 else 0.98
                threshold_pool[category][1] = get_GMM_threshold(all_conf_score[category])[1] if get_GMM_threshold(all_conf_score[category])[1] > 0.95 else 0.95
            if len(all_roi_score[category]) > 2:
                threshold_pool[category][2] = get_GMM_threshold(all_roi_score[category])[1] if get_GMM_threshold(all_roi_score[category])[1] > 0.85 else 0.85
                threshold_pool[category][3] = get_GMM_threshold(all_roi_score[category])[1] if get_GMM_threshold(all_roi_score[category])[1] > 0.91 else 0.91
            if len(all_iou_score[category]) > 2:
                threshold_pool[category][4] = get_GMM_threshold(all_iou_score[category])[1] if get_GMM_threshold(all_iou_score[category])[1] > 0.87 else 0.87
                threshold_pool[category][5] = get_GMM_threshold(all_iou_score[category])[1] if get_GMM_threshold(all_iou_score[category])[1] > 0.9 else 0.9

        if category == 'Cyclist':
            if len(all_conf_score[category]) > 2:
                threshold_pool[category][0] = get_GMM_threshold(all_conf_score[category])[1] if get_GMM_threshold(all_conf_score[category])[1] >= 0.97 else 0.97
                threshold_pool[category][1] = get_GMM_threshold(all_conf_score[category])[1]
            if len(all_roi_score[category]) > 2:
                threshold_pool[category][2] = get_GMM_threshold(all_roi_score[category])[1] if get_GMM_threshold(all_roi_score[category])[1] >= 0.91 else 0.91
                threshold_pool[category][3] = get_GMM_threshold(all_roi_score[category])[1]
            if len(all_roi_score[category]) > 2:
                threshold_pool[category][4] = get_GMM_threshold(all_iou_score[category])[1] if get_GMM_threshold(all_iou_score[category])[1] >= 0.84 else 0.84
                threshold_pool[category][5] = get_GMM_threshold(all_iou_score[category])[1]
